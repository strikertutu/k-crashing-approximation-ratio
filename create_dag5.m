...
function G = create_dag5(V,b_min,b_max,c_min,c_max,Lr,L1r,patience)
%% create original graph
%tic;

B=zeros(V,V);
layer_size = 20;
l_num = floor((V-2)/layer_size);
%disp(l_num);
for k=1:l_num
    for k1 = 1:layer_size*3
        % randomly create i<j
        i_l = (k-1)*layer_size+2;
        i_h = min([(k)*layer_size+1,V-1]);
        j_l = min([(k)*layer_size+2,V]);
        j_h = min([(k+1)*layer_size+1,V]);
        i=randi([i_l,i_h]);
        j=randi([j_l,j_h]);
        % connect i to j
        if B(i,j)==0
