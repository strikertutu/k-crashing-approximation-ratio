...
%% LOOP
for k_option = 1:1
    D=days(k_option);            % days to squeeze
    iter=0;
    res = zeros(1,maxnum);
    abnormal = zeros(0,0);
    while iter<maxnum
        GenStart = tic;
        G=create_dag12(V,b_min,b_max,c_min,c_max,Lr,L1r,E);
    ...
    if(size(abnormal))
        ...
        filename = ['abnormal-', num2str(D), '.xlsx'];
        writematrix(abnormal, filename);
        ...
        title(['Cumulative Distribution of Approximation Ratio when k=', num2str(D)]);

        exportgraphics(f, ['abnormal-', num2str(D), '.pdf'], 'ContentType', 'vector');
    end
end 
