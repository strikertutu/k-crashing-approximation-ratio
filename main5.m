...
%% LOOP
batchsize = 10;
times = zeros((Et-Es)/step+1,batchsize,2);
iter = 1;
res1 = zeros((Et-Es)/step+1,2);
res2 = zeros((Et-Es)/step+1,2);
for j = 1:(Et-Es)/step+1
    i = (Et-Es)/step+1-j+1;
    disp(E);  
    for k = 1:batchsize
        disp(["iteration:",iter])
        GenStart = tic;
        G=create_dag7(V,b_min,b_max,c_min,c_max,Lr,L1r,E);
